...
public class TypeInferencer implements Visitor1<Type, State> {
    ...
    private Type resolveUnion( Collection<? extends Node> nodes, State s) {
    ...
    public void setAttr(Attribute node, State s,  Type v) {
        Type targetType = visit(node.target, s);
        if (targetType instanceof UnionType) {
            Set<Type> types = ((UnionType) targetType).types;
            for (Type tp : types) {
    ...
    private void setAttrType(Attribute node,  Type targetType,  Type v)
    {
    ...
    public Type getAttrType(Attribute node,  Type targetType)
    {
        Set<Binding> bs = targetType.table.lookupAttr(node.attr.id);
        if (bs == null)
        {
    ...
    public Type resolveCall( Type fun,
                             Type selfType,
    ...
    public Type apply( FunType func,
                       Type selfType,
                       List<Type> positional,
                       Map<String, Type> kwTypes,
                       Type kwArg,
    ...
    private Type bindParams( State state,
                             FunctionDef func,
                             List<Type> pTypes,
                             List<Type> dTypes,
    ...
    static void bindMethodAttrs( FunType cl) {
    ...
    static void addReadOnlyAttr( FunType fun,
                                String name,
                                 Type type,
                                Binding.Kind kind) {
    ...
    public Type getSubscript(Node node,  Type vt,  Type st, State s) {
        if (vt.isUnknownType()) {
            return Types.UNKNOWN;
        } else {
    ...
    private Type getListSubscript(Node node,  Type vt,  Type st, State s) {
        if (vt instanceof ListType) {
    ...
    public void bind( State s, Node target,  Type rvalue, Binding.Kind kind)
    {
     ...
     * of the scope.
     */
    public void bind( State s, Node target,  Type rvalue) {
        Binding.Kind kind;
    ...
    public void bind( State s,  List<Node> xs,  Type rvalue, Binding.Kind kind) {
    ...
    public static void bind( State s,  Name name,  Type rvalue, Binding.Kind kind) {
    ...
    // iterator
    public void bindIter( State s, Node target,  Node iter, Binding.Kind kind) {
